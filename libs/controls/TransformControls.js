	...
	THREE.TransformControls = function ( camera, domElement ) {
		...
		this.update = function () {
            try{
                ...
                var cp= scope.object;
                guicontrols.X坐标=cp.position.z;
                guicontrols.Y坐标=cp.position.x;
                guicontrols.Z坐标=cp.position.y;
                guicontrols.X角度=cp.rotation.z/Math.PI*180.0;
                guicontrols.Y角度=cp.rotation.x/Math.PI*180.0;
                guicontrols.Z角度=cp.rotation.y/Math.PI*180.0;
                guicontrols.X倍数=cp.scale.z;
                guicontrols.Y倍数=cp.scale.x;
                guicontrols.Z倍数=cp.scale.y;
            }catch(event){
            }
			...
			if ( pointer.button === 0 || pointer.button === undefined ) {
				...
				if ( intersect ) {
					...
					if ( planeIntersect ) {
                        ...
                        if (scope.object.parent!=null){///////////修改hpd////////////////////2016
